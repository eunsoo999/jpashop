...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception {
        ...
        Book book = createBook("해리포터", 10000, 10);

        //when
        int orderCount = 2;
        Long orderId = orderService.order(member.getId(), book.getId(), orderCount); // 2권 주문

        //then
        Order getOrder = orderRepository.findOne(orderId);

        assertEquals("상품 주문시 상태는 ORDER", OrderStatus.ORDER, getOrder.getStatus()); // 메시지, 기대하는 값, 실제 값
    ...
    public void 주문취소() throws Exception {
        //given
        Member member = createMember();
        Book item = createBook("해리포터", 10000, 10);
        ...
        assertEquals("주문 취소시 사애는 CANCEL 입니다", OrderStatus.CANCEL, getOrder.getStatus());
    ...
    private Member createMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("서울", "랄랄로", "123-123"));
        em.persist(member);
        return member;
    }
